...
static tavros::core::logger logger("tr_main");
...
*/
int32 R_CullLocalPointAndRadius(vec3_t pt, float radius)
{
    vec3_t transformed;
...
*/
int32 R_CullPointAndRadius(vec3_t pt, float radius)
{
    int32     i;
    float     dist;
    cplane_t* frust;
...
void R_PlaneForSurface(surfaceType_t* surfType, cplane_t* plane)
{
    ...
    case SF_TRIANGLES:
        ...
        VectorCopy(plane4.data(), plane->normal);
        plane->dist = plane4[3];
        return;
    case SF_POLY:
...
=================
*/
bool R_GetPortalOrientations(drawSurf_t* drawSurf, int32 entityNum, orientation_t* surface, orientation_t* camera, vec3_t pvsOrigin, bool* mirror)
{
    int32          i;
    ...
    logger.info("Portal surface without a portal entity");

    return false;
}

static bool IsMirror(const drawSurf_t* drawSurf, int32 entityNum)
{
    int32          i;
    cplane_t       originalPlane, plane;
    trRefEntity_t* e;
...
*/
static bool SurfIsOffscreen(const drawSurf_t* drawSurf, tavros::math::vec4 clipDest[128])
{
    ...
    int32              fogNum;
    int32              dlighted;
    tavros::math::vec4 clip, eye;
    int32              i;
    uint32             pointOr = 0;
    uint32             pointAnd = (uint32) ~0;
    ...
    for (i = 0; i < tess.numVertexes; i++) {
        ...
        R_TransformModelToClip(tess.xyz[i].data(), tr.orient.modelMatrix, tr.viewParms.projectionMatrix, eye, clip);
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
        ...
        VectorSubtract(tess.xyz[tess.indexes[i]].data(), tr.viewParms.orient.origin, normal);
        ...
        if ((dot = DotProduct(normal, tess.normal[tess.indexes[i]].data())) >= 0) {
            numTriangles--;
        }
    }
    if (!numTriangles) {
...
========================
*/
bool R_MirrorViewBySurface(drawSurf_t* drawSurf, int32 entityNum)
{
    tavros::math::vec4 clipDest[128];
    ...
    if (tr.viewParms.isPortal) {
        logger.info("WARNING: recursive mirror/portal found");
        return false;
    }
...
=================
*/
int32 R_SpriteFogNum(trRefEntity_t* ent)
{
    int32  i, j;
    fog_t* fog;
...
)
{
    ...
    uint32 size;          /* size of the sub-array */
    char * lostk[30], *histk[30];
    int32  stkptr;        /* stack for saving sub-array to be processed */
    int32  temp;

    if (sizeof(drawSurf_t) != sizeof(void*) * 2) {
        Com_Error(ERR_DROP, "change swap_draw_surf() func");
    }
...
void R_AddDrawSurf(surfaceType_t* surface, shader_t* shader, int32 fogIndex, int32 dlightMap)
{
    ...
    tr.refdef.drawSurfs[index].sort = (shader->sortedIndex << QSORT_SHADERNUM_SHIFT)
                                    | tr.shiftedEntityNum | (fogIndex << QSORT_FOGNUM_SHIFT) | (int32) dlightMap;
...
=================
*/
void R_SortDrawSurfs(drawSurf_t* drawSurfs, int32 numDrawSurfs)
{
    shader_t* shader;
    int32     fogNum;
    int32     entityNum;
...
================
*/
void R_DebugPolygon(int32 color, int32 numPoints, float* points)
{
    int32 i;
