...
================
*/
static void AddSkyPolygon(int32 nump, vec3_t vecs)
{
    int32  i, j;
    vec3_t v, av;
    float  s, t, dv;
...
================
*/
static void ClipSkyPolygon(int32 nump, vec3_t vecs, int32 stage)
{
    float* norm;
    float* v;
    bool   front, back;
...
void RB_ClipSkyPolygons(shaderCommands_t* input)
{
    ...
    for (i = 0; i < input->numIndexes; i += 3) {
        for (j = 0; j < 3; j++) {
            VectorSubtract(input->xyz[input->indexes[i + j]].data(), backEnd.viewParms.orient.origin, p[j]);
        }
...
*/
static void MakeSkyVec(float s, float t, int32 axis, float outSt[2], vec3_t outXYZ)
{
    // 1 = s, 2 = t, 3 = 2048
    static int32 st_to_vec[6][3] =
        {
            {3, -1, 2},
            {-3, 1, 2},
...
static int32  sky_texorder[6] = {0, 2, 1, 3, 4, 5};
static vec3_t s_skyPoints[SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS + 1];
static float  s_skyTexCoords[SKY_SUBDIVISIONS + 1][SKY_SUBDIVISIONS + 1][2];

static void DrawSkySide(texture_type image, const int32 mins[2], const int32 maxs[2])
{
    int32 s, t;
...
static void DrawSkyBox(shader_t* shader)
{
    ...
    for (i = 0; i < 6; i++) {
        int32 sky_mins_subd[2], sky_maxs_subd[2];
        int32 s, t;
...
static void FillCloudySkySide(const int32 mins[2], const int32 maxs[2], bool addIndexes)
{
    int32 s, t;
    int32 vertexStart = tess.numVertexes;
    int32 tHeight, sWidth;
    ...
    for (t = mins[1] + HALF_SKY_SUBDIVISIONS; t <= maxs[1] + HALF_SKY_SUBDIVISIONS; t++) {
        for (s = mins[0] + HALF_SKY_SUBDIVISIONS; s <= maxs[0] + HALF_SKY_SUBDIVISIONS; s++) {
            VectorAdd(s_skyPoints[t][s], backEnd.viewParms.orient.origin, tess.xyz[tess.numVertexes].data());
            tess._texCoords[tess.numVertexes][0] = tavros::math::vec2(s_skyTexCoords[t][s][0], s_skyTexCoords[t][s][1]);
...
static void FillCloudBox(const shader_t* shader, int32 stage)
{
    int32 i;
...
void RB_StageIteratorSky()
{
    ...
    // Sky should be drawn only in the background
