...
static tavros::core::logger logger("tr_shade_calc");
...
{
    int32        i;
    const float* st = tess._texCoords[0]->data();
    float*       xyz = (float*) tess.xyz;
    float*       normal = (float*) tess.normal;
    float        now;
    ...
    for (i = 0; i < tess.numVertexes; i++, xyz += 4, st += 4, normal += 4) {
        ...
        scale = sinf(st[0] * ds->bulgeWidth + now) * ds->bulgeHeight;
...
void DeformText(const char* text)
{
    ...
    height[1] = 0;
    height[2] = -1;
    CrossProduct(tess.normal[0].data(), height, width);
    ...
    top = -999999;
    for (i = 0; i < 4; i++) {
        VectorAdd(tess.xyz[i].data(), mid, mid);
...
void RB_CalcColorFromEntity(uint8* dstColors)
{
    ...
    c = *(int32*) backEnd.currentEntity->e.shaderRGBA;
...
*/
void RB_CalcModulateColorsByFog(uint8* colors)
{
    int32 i;
...
*/
void RB_CalcModulateAlphasByFog(uint8* colors)
{
    int32 i;
    float texCoords[SHADER_MAX_VERTEXES][2];
...
*/
void RB_CalcModulateRGBAsByFog(uint8* colors)
{
    int32 i;
    float texCoords[SHADER_MAX_VERTEXES][2];
...
void RB_CalcFogTexCoords(float* st)
{
    ...
    fog_t*             fog;
    vec3_t             local;
    tavros::math::vec4 fogDistanceVector, fogDepthVector;
    ...
    if (fog->hasSurface) {
        fogDepthVector[0] = fog->surface[0] * backEnd.orient.axis[0][0] + fog->surface[1] * backEnd.orient.axis[0][1] + fog->surface[2] * backEnd.orient.axis[0][2];
        fogDepthVector[1] = fog->surface[0] * backEnd.orient.axis[1][0] + fog->surface[1] * backEnd.orient.axis[1][1] + fog->surface[2] * backEnd.orient.axis[1][2];
        fogDepthVector[2] = fog->surface[0] * backEnd.orient.axis[2][0] + fog->surface[1] * backEnd.orient.axis[2][1] + fog->surface[2] * backEnd.orient.axis[2][2];
        fogDepthVector[3] = -fog->surface[3] + DotProduct(backEnd.orient.origin, fog->surface);

        eyeT = DotProduct(backEnd.orient.viewOrigin, fogDepthVector.data()) + fogDepthVector[3];
    } else {
    ...
    for (i = 0, v = tess.xyz[0].data(); i < tess.numVertexes; i++, v += 4) {
        // calculate the length in fog
        s = DotProduct(v, fogDistanceVector.data()) + fogDistanceVector[3];
        t = DotProduct(v, fogDepthVector.data()) + fogDepthVector[3];
...
void RB_CalcTurbulentTexCoords(const waveForm_t* wf, float* st)
{
    ...
    for (i = 0; i < tess.numVertexes; i++, st += 2) {
        ...
        st[0] = s + sinf(((tess.xyz[i][0] + tess.xyz[i][2]) * 1.0 / 128 * 0.125 + now) * 2.0f * M_PI) * wf->amplitude;
        st[1] = t + sinf((tess.xyz[i][1] * 1.0 / 128 * 0.125 + now) * 2.0f * M_PI) * wf->amplitude;
    }
}
...
void RB_CalcRotateTexCoords(float degsPerSecond, float* st)
{
    ...
    sinValue = sinf(DEG_TO_RAD(degs));
    cosValue = cosf(DEG_TO_RAD(degs));
...
void RB_CalcDiffuseColor(uint8* colors)
{
    ...
    for (i = 0; i < numVertexes; i++, v += 4, normal += 4) {
        ...
        if (incoming <= 0) {
            *(int32*) &colors[i * 4] = ambientLightInt;
            continue;
        }
