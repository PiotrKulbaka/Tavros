...
{
    int32               number;      // number of the level item
    int32               iteminfo;    // index into the item info
    int32               flags;       // item flags
    float               weight;      // fixed roam weight
    vec3_t              origin;      // origin of the item
    int32               goalareanum; // area the item is in
    vec3_t              goalorigin;  // goal origin within the area
    int32               entitynum;   // entity number
...
typedef struct iteminfo_s
{
    ...
    int32  modelindex;             // model index
    int32  type;                   // item type
    int32  index;                  // index in the inventory
    float  respawntime;            // respawn time
    vec3_t mins;                   // mins of the item
    vec3_t maxs;                   // maxs of the item
    int32  number;                 // number of the item info
} iteminfo_t;

#define ITEMINFO_OFS(x) (int32) & (((iteminfo_t*) 0)->x)
...
typedef struct bot_goalstate_s
{
    ...
    int32*                 itemweightindex;  // index from item to weight
    //
    int32 client;               // client using this goal state
    int32 lastreachabilityarea; // last area with reachabilities the bot was in
    //
    bot_goal_t goalstack[MAX_GOALSTACK]; // goal stack
    int32      goalstacktop;             // the top of the goal stack
    //
    int32 avoidgoals[MAX_AVOIDGOALS];     // goals to avoid
...
itemconfig_t* LoadItemConfig(char* filename)
{
    ...
    max_iteminfo = (int32) LibVarValue("max_iteminfo", "256");
    if (max_iteminfo < 0) {
...
// Changes Globals:        -
//===========================================================================
int32* ItemWeightIndex(weightconfig_t* iwc, itemconfig_t* ic)
{
    int32 *index, i;

    // initialize item weight index
    index = (int32*) GetClearedMemory(sizeof(int32) * ic->numiteminfo);
...
{
    int32 i, max_levelitems;

    if (levelitemheap) {
        FreeMemory(levelitemheap);
    }

    max_levelitems = (int32) LibVarValue("max_levelitems", "256");
...
void BotInitInfoEntities()
{
    ...
    maplocation_t* ml;
    campspot_t*    cs;
    int32          ent, numlocations, numcampspots;
...
{
    int32         i, spawnflags, value;
    char          classname[MAX_EPAIRKEY];
    vec3_t        origin, end;
    int32         ent, goalareanum;
    itemconfig_t* ic;
    levelitem_t*  li;
    bsp_trace_t   trace;
...
void BotResetAvoidGoals(int32 goalstate)
{
    ...
    Com_Memset(gs->avoidgoals, 0, MAX_AVOIDGOALS * sizeof(int32));
...
void BotAddToAvoidGoals(bot_goalstate_t* gs, int32 number, float avoidtime)
{
    int32 i;
...
// Changes Globals:        -
//===========================================================================
int32 BotGetLevelItemGoal(int32 index, char* name, bot_goal_t* goal)
{
    levelitem_t* li;
    ...
    for (; li; li = li->next) {
        //
        if (_g_gametype == GT_SINGLE_PLAYER) {
            if (li->flags & IFL_NOTSINGLE) {
                continue;
            }
        } else if (_g_gametype >= GT_TEAM) {
...
{
    int32            areanum, t, weightnum, ltg_time;
    float            weight, bestweight, avoidtime;
    iteminfo_t*      iteminfo;
    itemconfig_t*    ic;
    levelitem_t *    li, *bestitem;
...
int32 BotSetupGoalAI()
{
    ...
    _g_gametype = LibVarValue("g_gametype", "0");
