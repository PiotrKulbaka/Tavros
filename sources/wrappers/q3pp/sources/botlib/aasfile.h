...
typedef struct aas_areasettings_s
{
    ...
    int32 contents;           // contents of the area
    int32 areaflags;          // several area flags
    int32 presencetype;       // how a bot can be present in this area
    int32 cluster;            // cluster the area belongs to, if negative it's a portal
    int32 clusterareanum;     // number of the area in the cluster
    int32 numreachableareas;  // number of reachable areas from this one
    int32 firstreachablearea; // first reachable area in the reachable area index
} aas_areasettings_t;

// cluster portal
typedef struct aas_portal_s
{
    int32 areanum;           // area that is the actual portal
    int32 frontcluster;      // cluster at front of portal
    int32 backcluster;       // cluster at back of portal
    int32 clusterareanum[2]; // number of the area in the front and back cluster
} aas_portal_t;

// cluster portal index
typedef int32 aas_portalindex_t;

// cluster
typedef struct aas_cluster_s
{
    int32 numareas;             // number of areas in the cluster
    int32 numreachabilityareas; // number of areas with reachabilities
    int32 numportals;           // number of cluster portals
    int32 firstportal;          // first cluster portal in the index
} aas_cluster_t;
...
{
    int32 v[2]; // numbers of the vertexes of this edge
} aas_edge_t;

// edge index, negative if vertexes are reversed
typedef int32 aas_edgeindex_t;

// a face bounds an area, often it will also seperate two areas
typedef struct aas_face_s
{
    int32 planenum;  // number of the plane this face is in
    int32 faceflags; // face flags (no use to create face settings for just this field)
    int32 numedges;  // number of edges in the boundary of the face
    int32 firstedge; // first edge in the edge index
    int32 frontarea; // area at the front of this face
    int32 backarea;  // area at the back of this face
} aas_face_t;

// face index, stores a negative index if backside of face
typedef int32 aas_faceindex_t;

// area with a boundary of faces
typedef struct aas_area_s
{
    int32 areanum; // number of this area
    // 3d definition
    int32  numfaces;  // number of faces used for the boundary of the area
    int32  firstface; // first face in the face index used for the boundary of the area
...
{
    int32 planenum;
    int32 children[2]; // child nodes of this node, or areas as leaves when negative
