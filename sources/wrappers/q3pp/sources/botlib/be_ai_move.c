...
typedef struct bot_movestate_s
{
    ...
    int32  presencetype; // presencetype of the bot
    vec3_t viewangles;   // view angles of the bot
    // state vars
    int32  areanum;                         // area the bot is in
    int32  lastareanum;                     // last area the bot was in
    int32  lastgoalareanum;                 // last goal area number
    int32  lastreachnum;                    // last reachability number
    vec3_t lastorigin;                      // origin previous cycle
    int32  reachareanum;                    // area number of the reachabilty
    int32  moveflags;                       // movement flags
    int32  jumpreach;                       // set when jumped
    ...
    int32  avoidreach[MAX_AVOIDREACH];      // reachabilities to avoid
    float  avoidreachtimes[MAX_AVOIDREACH]; // times to avoid the reachabilities
    int32  avoidreachtries[MAX_AVOIDREACH]; // number of tries before avoiding
    //
...
bot_movestate_t* BotMoveStateFromHandle(int32 handle)
{
    if (handle <= 0 || handle > MAX_CLIENTS) {
        botimport.Print(PRT_FATAL, "move state handle %d out of range\n", handle);
        return NULL;
...
// Changes Globals:        -
//========================================================================
void BotInitMoveState(int32 handle, bot_initmove_t* initmove)
{
    bot_movestate_t* ms;
...
{
    int32  firstareanum, j, x, y, z;
    int32  areas[10], numareas, areanum, bestareanum;
    float  dist, bestdist;
...
// Changes Globals:        -
//===========================================================================
int32 BotReachabilityArea(vec3_t origin, int32 client)
{
    int32              modelnum, modeltype, reachnum, areanum;
...
// Changes Globals:        -
//===========================================================================
int32 BotOnMover(vec3_t origin, int32 entnum, aas_reachability_t* reach)
{
    int32       i, modelnum;
...
// Changes Globals:        -
//===========================================================================
int32 MoverDown(aas_reachability_t* reach)
{
    int32  modelnum;
...
void BotSetBrushModelTypes()
{
    ...
    Com_Memset(modeltypes, 0, MAX_MODELS * sizeof(int32));
    //
...
// Changes Globals:        -
//===========================================================================
int32 BotOnTopOfEntity(bot_movestate_t* ms)
{
...
// Changes Globals:        -
//===========================================================================
int32 BotValidTravel(vec3_t origin, aas_reachability_t* reach, int32 travelflags)
{
...
void BotAddToAvoidReach(bot_movestate_t* ms, int32 number, float avoidtime)
{
    int32 i;
...
// Changes Globals:        -
//===========================================================================
int32 BotAvoidSpots(vec3_t origin, aas_reachability_t* reach, bot_avoidspot_t* avoidspots, int32 numavoidspots)
{
    int32 checkbetween, i, type;
...
{
    int32              i, t, besttime, bestreachnum, reachnum;
    aas_reachability_t reach;
...
int32 BotAddToTarget(vec3_t start, vec3_t end, float maxdist, float* dist, vec3_t target)
{
    vec3_t dir;
    float  curdist;
...
int32 BotMovementViewTarget(int32 movestate, bot_goal_t* goal, int32 travelflags, float lookahead, vec3_t target)
{
    aas_reachability_t reach;
    int32              reachnum, lastareanum;
    bot_movestate_t*   ms;
    vec3_t             end;
    float              dist;
...
// Changes Globals:        -
//===========================================================================
int32 BotVisible(int32 ent, vec3_t eye, vec3_t target)
{
    bsp_trace_t trace;
...
// Changes Globals:        -
//===========================================================================
int32 BotPredictVisiblePosition(vec3_t origin, int32 areanum, bot_goal_t* goal, int32 travelflags, vec3_t target)
{
    aas_reachability_t reach;
    int32              reachnum, lastgoalareanum, lastareanum, i;
    int32              avoidreach[MAX_AVOIDREACH];
    float              avoidreachtimes[MAX_AVOIDREACH];
    int32              avoidreachtries[MAX_AVOIDREACH];
    vec3_t             end;
    ...
    Com_Memset(avoidreach, 0, MAX_AVOIDREACH * sizeof(int32));
...
// Changes Globals:        -
//===========================================================================
int32 BotCheckBarrierJump(bot_movestate_t* ms, vec3_t dir, float speed)
{
...
// Changes Globals:        -
//===========================================================================
int32 BotSwimInDirection(bot_movestate_t* ms, vec3_t dir, float speed, int32 type)
{
    vec3_t normdir;
...
// Changes Globals:        -
//===========================================================================
int32 BotWalkInDirection(bot_movestate_t* ms, vec3_t dir, float speed, int32 type)
{
    vec3_t           hordir, cmdmove, velocity, tmpdir, origin;
    int32            presencetype, maxframes, cmdframes, stopevent;
    ...
    if (ms->moveflags & MFL_ONGROUND) {
        ...
        origin[2] += 0.5;
        AAS_PredictClientMovement(&move, ms->entitynum, origin, presencetype, true, velocity, cmdmove, cmdframes, maxframes, 0.1f, stopevent, 0, false); // true);
...
// Changes Globals:        -
//===========================================================================
void BotCheckBlocked(bot_movestate_t* ms, vec3_t dir, int32 checkbottom, bot_moveresult_t* result)
{
    vec3_t      mins, maxs, end, up = {0, 0, 1};
    bsp_trace_t trace;
...
// Changes Globals:        -
//===========================================================================
int32 BotAirControl(vec3_t origin, vec3_t velocity, vec3_t goal, vec3_t dir, float* speed)
{
    vec3_t org, vel;
    float  dist;
    int32  i;
...
// Changes Globals:        -
//===========================================================================
int32 GrappleState(bot_movestate_t* ms, aas_reachability_t* reach)
{
    int32            i;
    ...
    for (i = AAS_NextEntity(0); i; i = AAS_NextEntity(i)) {
        if (AAS_EntityType(i) == (int32) entitytypemissile->value) {
            AAS_EntityInfo(i, &entinfo);
            if (entinfo.weapon == (int32) weapindex_grapple->value) {
                return 1;
            }
        }
    }
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
    ...
    EA_SelectWeapon(ms->client, (int32) weapindex_rocketlauncher->value);
    // weapon is used for movement
    result.weapon = (int32) weapindex_rocketlauncher->value;
    result.flags |= MOVERESULT_MOVEMENTWEAPON;
    //
    VectorCopy(hordir, result.movedir);
    //
...
bot_moveresult_t BotTravel_BFGJump(bot_movestate_t* ms, aas_reachability_t* reach)
{
    ...
    result.flags |= MOVERESULT_MOVEMENTVIEWSET;
    // select the rocket launcher
    EA_SelectWeapon(ms->client, (int32) weapindex_bfg10k->value);
    // weapon is used for movement
    result.weapon = (int32) weapindex_bfg10k->value;
    result.flags |= MOVERESULT_MOVEMENTWEAPON;
    //
    VectorCopy(hordir, result.movedir);
    //
...
// Changes Globals:        -
//===========================================================================
int32 BotReachabilityTime(aas_reachability_t* reach)
{
    switch (reach->traveltype & TRAVELTYPE_MASK) {
    case TRAVEL_WALK:
        return 5;
...
void BotMoveToGoal(bot_moveresult_t* result, int32 movestate, bot_goal_t* goal, int32 travelflags)
{
    int32              reachnum, lastreachnum, foundjumppad, ent, resultflags;
...
void BotResetAvoidReach(int32 movestate)
{
    ...
    Com_Memset(ms->avoidreach, 0, MAX_AVOIDREACH * sizeof(int32));
    Com_Memset(ms->avoidreachtimes, 0, MAX_AVOIDREACH * sizeof(float));
    Com_Memset(ms->avoidreachtries, 0, MAX_AVOIDREACH * sizeof(int32));
}
