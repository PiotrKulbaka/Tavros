...
int32 AAS_ClipToBBox(aas_trace_t* trace, vec3_t start, vec3_t end, int32 presencetype, vec3_t mins, vec3_t maxs)
{
    int32  i, j, side;
...
// Changes Globals:        -
int32 AAS_ClientMovementPrediction(struct aas_clientmove_s* move, int32 entnum, vec3_t origin, int32 presencetype, int32 onground, vec3_t velocity, vec3_t cmdmove, int32 cmdframes, int32 maxframes, float frametime, int32 stopevent, int32 stopareanum, vec3_t mins, vec3_t maxs, int32 visualize)
{
    ...
    int32        n, i, j, pc, step, swimming, ax, crouch, event, jump_frame, areanum;
    int32        areas[20], numareas;
    vec3_t       points[20];
