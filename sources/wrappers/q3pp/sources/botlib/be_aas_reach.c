...
int32 reach_swim;         // swim
int32 reach_equalfloor;   // walk on floors with equal height
int32 reach_step;         // step up
int32 reach_walk;         // walk of step
int32 reach_barrier;      // jump up to a barrier
int32 reach_waterjump;    // jump out of water
int32 reach_walkoffledge; // walk of a ledge
int32 reach_jump;         // jump
int32 reach_ladder;       // climb or descent a ladder
int32 reach_teleport;     // teleport
int32 reach_elevator;     // use an elevator
int32 reach_funcbob;      // use a func bob
int32 reach_grapple;      // grapple hook
int32 reach_rocketjump;   // rocket jump
int32 reach_jumppad;      // jump pads
// if true grapple reachabilities are skipped
int32 calcgrapplereach;
// linked reachability
typedef struct aas_lreachability_s
{
    int32  areanum;    // number of the reachable area
    int32  facenum;    // number of the face towards the other area
    int32  edgenum;    // number of the edge towards the other area
    vec3_t start;      // start point of inter area movement
    vec3_t end;        // end point of inter area movement
    int32  traveltype; // type of travel required to get to the area
    uint16 traveltime; // travel time of the inter area movement
    //
...
float AAS_AreaVolume(int32 areanum)
{
    int32        i, edgenum, facenum, side;
    vec_t        d, a, volume;
    vec3_t       corner;
    aas_plane_t* plane;
    aas_edge_t*  edge;
...
int32 AAS_GetJumpPadInfo(int32 ent, vec3_t areastart, vec3_t absmins, vec3_t absmaxs, vec3_t velocity)
{
    int32       modelnum, ent2;
...
{
    int32            area2num, ent, bot_visualizejumppads, bestareanum;
...
void AAS_FaceCenter(int32 facenum, vec3_t center)
{
    int32       i;
    float       scale;
    aas_face_t* face;
...
int32 AAS_AreaTeleporter(int32 areanum)
{
    return (aasworld.areasettings[areanum].contents & AREACONTENTS_TELEPORTER);
}

int32 AAS_AreaClusterPortal(int32 areanum)
{
...
bool AAS_ReachabilityExists(int32 area1num, int32 area2num)
{
    aas_lreachability_t* r;
...
int32 AAS_Reachability_Swim(int32 area1num, int32 area2num)
{
    int32                i, j, face1num, face2num, side1;
...
// heights
int32 AAS_Reachability_EqualFloorHeight(int32 area1num, int32 area2num)
{
    int32               i, j, edgenum, edgenum1, edgenum2, foundreach, side;
...
int32 AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int32 area1num, int32 area2num)
{
    int32                i, j, k, l, edge1num, edge2num, areas[10], numareas;
    int32                ground_bestarea2groundedgenum, ground_foundreach;
    int32                water_bestarea2groundedgenum, water_foundreach;
    int32                side1, area1swim, faceside1, groundface1num;
            ...
            if (!aassettings.rs_maxfallheight || fabs(ground_bestdist) < aassettings.rs_maxfallheight) {
                ...
                if (!trace.startsolid && trace.fraction >= 1.0) {
                    ...
                    if (AAS_PointAreaNum(trace.endpos) == area2num) {
                        ...
                        numareas = AAS_TraceAreas(start, end, areas, NULL, sizeof(areas) / sizeof(int32));
...
int32 VectorBetweenVectors(vec3_t v, vec3_t v1, vec3_t v2)
{
    vec3_t dir1, dir2;
...
int32 AAS_Reachability_Jump(int32 area1num, int32 area2num)
{
    int32                i, j, k, l, face1num, face2num, edge1num, edge2num, traveltype;
    int32                stopevent, areas[10], numareas;
    ...
    if (bestdist > 4 && bestdist < maxjumpdistance) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            teststart[2] += 1;
            numareas = AAS_TraceAreas(move.endpos, teststart, areas, NULL, sizeof(areas) / sizeof(int32));
...
int32 AAS_Reachability_Ladder(int32 area1num, int32 area2num)
{
    int32                i, j, k, l, edge1num, edge2num, sharededgenum, lowestedgenum;
    int32                face1num, face2num, ladderface1num, ladderface2num;
    int32                ladderface1vertical, ladderface2vertical, firstv;
...
int32 AAS_TravelFlagsForTeam(int32 ent)
{
    int32 notteam;
    ...
    for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
        ...
        if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num)) {
...

            {
                ...
                VectorClear(cmdmove);
                AAS_PredictClientMovement(&move, -1, destorigin, PRESENCE_NORMAL, false, velocity, cmdmove, 0, 30, 0.1f, SE_HITGROUND | SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE | SE_TOUCHJUMPPAD | SE_TOUCHTELEPORTER, 0, false); // true);
...
{
    int32                area1num, area2num, modelnum, i, j, k, l, n, p;
...
aas_lreachability_t* AAS_FindFaceReachabilities(vec3_t* facepoints, int32 numpoints, aas_plane_t* plane, int32 towardsface)
{
    int32                i, j, k, l;
    int32                facenum, edgenum, bestfacenum;
    float *              v1, *v2, *v3, *v4;
...
{
    int32                ent, spawnflags, modelnum, axis;
    int32                i, numareas, areas[10];
    char                 classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
        ...
        for (i = 0; i < 2; i++) {
            ...
            for (startreach = firststartreach; startreach; startreach = nextstartreach) {
                ...
                for (endreach = firstendreach; endreach; endreach = nextendreach) {
                    ...
                    if (i == 0) {
                        lreach->edgenum = ((int32) move_start[axis] << 16) | ((int32) move_end[axis] & 0x0000ffff);
                    } else {
                        lreach->edgenum = ((int32) move_end[axis] << 16) | ((int32) move_start[axis] & 0x0000ffff);
                    }
...
{
    int32 face2num, i, ret, area2num, visualize, ent, bot_visualizejumppads;
    // int32 modelnum, ent2;
...
int32 AAS_Reachability_Grapple(int32 area1num, int32 area2num)
{
    int32                face2num, i, j, areanum, numareas, areas[20];
...
void AAS_SetWeaponJumpAreaFlags()
{
    ...
    vec3_t origin;
    int32  areanum, weaponjumpareas, spawnflags;
    char   classname[MAX_EPAIRKEY];
...
{
    int32                face2num, i, n, ret, visualize;
    float                speed, zvel, hordist;
    aas_face_t*          face2;
    aas_area_t *         area1, *area2;
    aas_lreachability_t* lreach;
...
void AAS_Reachability_WalkOffLedge(int32 areanum)
{
    int32                i, j, k, l, m, n, p, areas[10], numareas;
    int32                face1num, face2num, face3num, edge1num, edge2num, edge3num;
    int32                otherareanum, gap, reachareanum, side;
            ...
            for (j = 0; j < area->numfaces; j++) {
                ...
                for (l = 0; l < face2->numedges; l++) {
                    ...
                    if (abs(edge1num) == abs(edge2num)) {
                        ...
                        // if not going through a cluster portal
                        numareas = AAS_TraceAreas(mid, testend, areas, NULL, sizeof(areas) / sizeof(int32));
...
{
    int32        i, j, todo, start_time;
    static float framereachability, reachability_delay;
    static int32 lastpercentage;
    ...
    todo = aasworld.numreachabilityareas + (int32) framereachability;
    ...
    for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++) {
        ...
        if (Sys_MilliSeconds() - start_time > (int32) reachability_delay) {
            break;
        }
        //
...
void AAS_InitReachability()
{
    ...
    if (aasworld.reachabilitysize) {
        if (!((int32) LibVarGetValue("forcereachability"))) {
