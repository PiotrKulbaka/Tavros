...
static const char quake3worldMessage[] = "Visit www.quake3world.com - News, Community, Events, Files";
...
static int32        g_numglobalservers;
static servernode_t g_localserverlist[MAX_LOCALSERVERS];
static int32        g_numlocalservers;
static servernode_t g_favoriteserverlist[MAX_FAVORITESERVERS];
static int32        g_numfavoriteservers;
static servernode_t g_mplayerserverlist[MAX_GLOBALSERVERS];
static int32        g_nummplayerservers;
static int32        g_servertype;
static int32        g_gametype;
static int32        g_sortkey;
static int32        g_emptyservers;
static int32        g_fullservers;
...
=================
*/
static int32 ArenaServers_MaxPing()
{
    int32 maxPing;

    maxPing = (int32) Cvar_VariableValue("cl_maxPing");
    if (maxPing < 100) {
        maxPing = 100;
    }
    return maxPing;
...
====================
*/
static int32 LAN_GetServerCount(int32 source)
{
    switch (source) {
    case AS_LOCAL:
...
====================
*/
static void LAN_GetServerAddressString(int32 source, int32 n, char* buf, int32 buflen)
{
    switch (source) {
    case AS_LOCAL:
...
=================
*/
static int32 QDECL ArenaServers_Compare(const void* arg1, const void* arg2)
{
    float         f1;
    float         f2;
    servernode_t* t1;
...
static void ArenaServers_UpdateMenu()
{
    ...
    if (g_arenaservers.numqueriedservers > 0) {
        ...
        if (g_arenaservers.refreshservers && (g_arenaservers.currentping <= g_arenaservers.numqueriedservers)) {
            // show progress
            Com_sprintf((char*) g_arenaservers.status.string, MAX_STATUSLENGTH, "%d of %d Arena Servers.", g_arenaservers.currentping, g_arenaservers.numqueriedservers);
        ...
        if (g_arenaservers.refreshservers) {
            strcpy((char*) g_arenaservers.status.string, "Scanning For Servers.");
            g_arenaservers.statusbar.string = "Press SPACE to stop";
            ...
            if (g_arenaservers.numqueriedservers < 0) {
                strcpy((char*) g_arenaservers.status.string, "No Response From Master Server.");
            } else {
                strcpy((char*) g_arenaservers.status.string, "No Servers Found.");
            }
...
=================
*/
static void ArenaServers_Insert(char* adrstr, char* info, int32 pingtime)
{
    servernode_t* servernodeptr;
    char*         s;
    int32         i;
...
static void ArenaServers_DoRefresh()
{
    ...
    for (i = 0; i < MAX_PINGREQUESTS; i++) {
        CL_GetPing(i, adrstr, MAX_ADDRESSLENGTH, &time);
        if (!adrstr[0]) {
        ...
        if (j < MAX_PINGREQUESTS) {
            ...
            if (time > maxPing) {
                ...
                CL_GetPingInfo(i, info, MAX_INFO_STRING);
            }
    ...
    for (i = 0; i < MAX_PINGREQUESTS && g_arenaservers.currentping < g_arenaservers.numqueriedservers; i++) {
        if (CL_GetPingQueueCount() >= MAX_PINGREQUESTS) {
            // ping queue is full
            break;
        }
...
=================
*/
void ArenaServers_Sort(int32 type)
{
    if (g_sortkey == type) {
        return;
    }
...
=================
*/
void ArenaServers_SetType(int32 type)
{
    ...
    strcpy((char*) g_arenaservers.status.string, "hit refresh to update");
}
...
=================
*/
static void ArenaServers_Event(void* ptr, int32 event)
{
    int32 id;
    int32 value;
    ...
    case ID_PUNKBUSTER:
        if (g_arenaservers.punkbuster.curvalue) {
            UI_ConfirmMenu_Style("Enable Punkbuster?", UI_CENTER | UI_INVERSE | UI_SMALLFONT, NULL, Punkbuster_ConfirmEnable);
        } else {
            UI_ConfirmMenu_Style("Disable Punkbuster?", UI_CENTER | UI_INVERSE | UI_SMALLFONT, NULL, Punkbuster_ConfirmDisable);
        }
        break;
    }
}
...
=================
*/
static sfxHandle_t ArenaServers_MenuKey(int32 key)
{
...
static void ArenaServers_MenuInit()
{
    ...
    g_servertype = Com_Clamp(0, 3, Cvar_VariableIntegerValue("ui_browserMaster"));
    ...
    g_gametype = Com_Clamp(0, 4, Cvar_VariableIntegerValue("ui_browserGameType"));
    g_arenaservers.gametype.curvalue = g_gametype;

    g_sortkey = Com_Clamp(0, 4, Cvar_VariableIntegerValue("ui_browserSortKey"));
    g_arenaservers.sortkey.curvalue = g_sortkey;

    g_fullservers = Com_Clamp(0, 1, Cvar_VariableIntegerValue("ui_browserShowFul"));
    g_arenaservers.showfull.curvalue = g_fullservers;

    g_emptyservers = Com_Clamp(0, 1, Cvar_VariableIntegerValue("ui_browserShowEmpty"));
