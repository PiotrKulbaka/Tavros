...
typedef struct
{
    ...
    int32 phase;
    int32 ignoreKeysTime;
    int32 starttime;
    int32 scoreboardtime;
    int32 serverId;

    int32 clientNums[MAX_SCOREBOARD_CLIENTS];
    int32 ranks[MAX_SCOREBOARD_CLIENTS];
    int32 scores[MAX_SCOREBOARD_CLIENTS];
    ...
    int32       won;
    int32       numAwards;
    int32       awardsEarned[6];
    int32       awardsLevels[6];
    bool        playedSound[6];
    int32       lastTier;
...
=================
*/
static void UI_SPPostgameMenu_AgainEvent(void* ptr, int32 event)
{
...
=================
*/
static void UI_SPPostgameMenu_NextEvent(void* ptr, int32 event)
{
    int32       currentSet;
    int32       levelSet;
    int32       level;
...
=================
*/
static void UI_SPPostgameMenu_MenuEvent(void* ptr, int32 event)
{
    if (event != QM_ACTIVATED) {
        return;
    }
...
=================
*/
static sfxHandle_t UI_SPPostgameMenu_MenuKey(int32 key)
{
...
static int32 medalLocations[6] = {144, 448, 88, 504, 32, 560};

static void UI_SPPostgameMenu_DrawAwardsMedals(int32 max)
{
    int32 n;
    int32 medal;
    int32 amount;
...
static void UI_SPPostgameMenu_DrawAwardsPresentation(int32 timer)
{
    ...
    if (!postgameMenuInfo.playedSound[awardNum]) {
        ...
        S_StartLocalSound(S_RegisterSound(ui_medalSounds[postgameMenuInfo.awardsEarned[awardNum]], false), CHAN_ANNOUNCER);
    }
}
...
=================
*/
static void UI_SPPostgameMenu_MenuDrawScoreLine(int32 n, int32 y)
{
    int32 rank;
    char  name[64];
    ...
    __UI_GetConfigString(CS_PLAYERS + postgameMenuInfo.clientNums[n], info, MAX_INFO_STRING);
...
static void UI_SPPostgameMenu_MenuDraw()
{
    ...
    __UI_GetConfigString(CS_SYSTEMINFO, info, sizeof(info));
    ...
    if (postgameMenuInfo.phase == 3) {
        if (postgameMenuInfo.won > -1) {
            ...
            Cbuf_ExecuteText(EXEC_APPEND, va("disconnect;\n"));
            return;
        }
...
void UI_SPPostgameMenu_Cache()
{
    ...
    for (n = 0; n < 6; n++) {
        ...
        S_RegisterSound(ui_medalSounds[n], false);
    }
}
...
static void Prepname(int32 index)
{
    int32 len;
    char  name[64];
    char  info[MAX_INFO_STRING];

    __UI_GetConfigString(CS_PLAYERS + postgameMenuInfo.clientNums[index], info, MAX_INFO_STRING);
    Q_strncpyz(name, Info_ValueForKey(info, "n"), sizeof(name));
    Q_CleanStr(name);
    len = strlen(name);
...
{
    int32       playerGameRank;
    int32       playerClientNum;
    int32       n;
    int32       oldFrags, newFrags;
    const char* arena;
    int32       awardValues[6];
    char        map[MAX_QPATH];
    char        info[MAX_INFO_STRING];
    ...
    __UI_GetConfigString(CS_SERVERINFO, info, sizeof(info));
    ...
    if (playerGameRank != 1) {
        postgameMenuInfo.winnerSound = S_RegisterSound(va("sound/player/announce/%s_wins.wav", postgameMenuInfo.placeNames[0]), false);
        Cbuf_ExecuteText(EXEC_APPEND, "music music/loss\n");
    } else {
        postgameMenuInfo.winnerSound = S_RegisterSound("sound/player/announce/youwin.wav", false);
