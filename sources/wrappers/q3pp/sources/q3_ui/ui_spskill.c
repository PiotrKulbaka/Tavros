...
=================
*/
static void UI_SPSkillMenu_SkillEvent(void* ptr, int32 notification)
{
    int32 id;
    int32 skill;
    ...
    SetSkillColor((int32) Cvar_VariableValue("g_spSkill"), color_red);
...
=================
*/
static void UI_SPSkillMenu_FightEvent(void* ptr, int32 notification)
{
    if (notification != QM_ACTIVATED) {
        return;
    }
...
=================
*/
static void UI_SPSkillMenu_BackEvent(void* ptr, int32 notification)
{
    if (notification != QM_ACTIVATED) {
        return;
    }
...
=================
*/
static sfxHandle_t UI_SPSkillMenu_Key(int32 key)
{
...
void UI_SPSkillMenu_Cache()
{
    ...
    skillMenuInfo.nightmareSound = S_RegisterSound("sound/misc/nightmare.wav", false);
    skillMenuInfo.silenceSound = S_RegisterSound("sound/misc/silence.wav", false);
}
...
static void UI_SPSkillMenu_Init()
{
    ...
    skill = (int32) Com_Clamp(1, 5, Cvar_VariableValue("g_spSkill"));
    SetSkillColor(skill, color_white);
    skillMenuInfo.art_skillPic.shader = skillMenuInfo.skillpics[skill - 1];
    if (skill == 5) {
        S_StartLocalSound(skillMenuInfo.nightmareSound, CHAN_ANNOUNCER);
