...
static tavros::core::logger logger("ui_players");
...
===============
*/
static void UI_ForceLegsAnim(playerInfo_t* pi, int32 anim)
{
...
===============
*/
static void UI_SetLegsAnim(playerInfo_t* pi, int32 anim)
{
...
===============
*/
static void UI_ForceTorsoAnim(playerInfo_t* pi, int32 anim)
{
...
===============
*/
static void UI_SetTorsoAnim(playerInfo_t* pi, int32 anim)
{
...
======================
*/
static void UI_PositionEntityOnTag(refEntity_t* entity, const refEntity_t* parent, clipHandle_t parentModel, const char* tagName)
{
    int32         i;
...
======================
*/
static void UI_PositionRotatedEntityOnTag(refEntity_t* entity, const refEntity_t* parent, clipHandle_t parentModel, const char* tagName)
{
    int32         i;
    orientation_t lerped;
    vec3_t        tempAxis[3];
...
===============
*/
static void UI_SetLerpFrameAnimation(playerInfo_t* ci, lerpFrame_t* lf, int32 newAnimation)
{
    animation_t* anim;
    ...
    if (newAnimation < 0 || newAnimation >= MAX_ANIMATIONS) {
        Com_Error(ERR_DROP, va("Bad animation number: %i", newAnimation));
    }
...
===============
*/
static void UI_RunLerpFrame(playerInfo_t* ci, lerpFrame_t* lf, int32 newAnimation)
{
    int32        f;
    animation_t* anim;
...
===============
*/
static void UI_PlayerAnimation(playerInfo_t* pi, int32* legsOld, int32* legs, float* legsBackLerp, int32* torsoOld, int32* torso, float* torsoBackLerp)
{
    // legs animation
...
==================
*/
static void UI_SwingAngles(float destination, float swingTolerance, float clampTolerance, float speed, float* angle, bool* swinging)
{
    float swing;
    float move;
    float scale;
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t* pi, int32 time)
{
    ...
    refdef.fov_x = (int32) ((float) refdef.width / 640.0f * 90.0f);
    ...
    if (dp_realtime <= pi->muzzleFlashTime) {
        ...
        if (pi->flashDlightColor[0] || pi->flashDlightColor[1] || pi->flashDlightColor[2]) {
            RE_AddLightToScene(flash.origin, 200 + (rand() & 31), pi->flashDlightColor[0], pi->flashDlightColor[1], pi->flashDlightColor[2]);
        }
    }
...
static bool UI_ParseAnimationFile(const char* filename, animation_t* animations)
{
    ...
    if (len >= (sizeof(text) - 1)) {
        logger.warning("File %s too long", filename);
        return false;
    }
    FS_Read2(text, len, f);
    text[len] = 0;
    ...
    if (i != MAX_ANIMATIONS) {
        logger.warning("Error parsing animation file: %s", filename);
        return false;
    }
...
bool UI_RegisterClientModelname(playerInfo_t* pi, const char* modelSkinName)
{
    ...
    if (!UI_RegisterClientSkin(pi, modelName, skinName)) {
        if (!UI_RegisterClientSkin(pi, modelName, "default")) {
            logger.warning("Failed to load skin file: %s : %s", modelName, skinName);
            return false;
        }
    }
...
===============
*/
void UI_PlayerInfo_SetInfo(playerInfo_t* pi, int32 legsAnim, int32 torsoAnim, vec3_t viewAngles, vec3_t moveAngles, weapon_t weaponNumber, bool chat)
{
    int32    currentAnim;
    weapon_t weaponNum;
