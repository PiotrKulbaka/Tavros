...
static void UI_ReadableSize(char* buf, int32 bufsize, int32 value)
{
...
static void UI_PrintTime(char* buf, int32 bufsize, int32 time)
{
...
static void UI_DisplayDownloadInfo(const char* downloadName)
{
    ...
    int32       downloadSize, downloadCount, downloadTime;
    char        dlSizeBuf[64], totalSizeBuf[64], xferRateBuf[64], dlTimeBuf[64];
    int32       xferRate;
    int32       width, leftWidth;
    int32       style = UI_LEFT | UI_SMALLFONT | UI_DROPSHADOW;
    const char* s;
    ...
    if (downloadCount < 4096 || !downloadTime) {
        ...
        if ((uis.realtime - downloadTime) / 1000) {
            ...
            // xferRate = (int32)( ((float)downloadCount) / elapsedTime);
        } else {
            xferRate = 0;
        }
        ...
        if (downloadSize && xferRate) {
            int32 n = downloadSize / xferRate; // estimated time for entire d/l in secs
...
void UI_DrawConnectScreen(bool overlay)
{
    ...
    __UI_GetClientState(&cstate);

    info[0] = '\0';
    if (__UI_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
