...
static sfxHandle_t RadioButton_Key(menuradiobutton_s* rb, int32 key);

// slider widget
static void        Slider_Init(menuslider_s* s);
static sfxHandle_t Slider_Key(menuslider_s* s, int32 key);
static void        Slider_Draw(menuslider_s* s);

// spin control widget
static void        SpinControl_Init(menulist_s* s);
static void        SpinControl_Draw(menulist_s* s);
static sfxHandle_t SpinControl_Key(menulist_s* l, int32 key);
...
sfxHandle_t ScrollList_Key(menulist_s* l, int32 key);
...
static void Slider_Draw(menuslider_s* s)
{
    ...
    UI_DrawHandlePic((int32) (x + 2 * SMALLCHAR_WIDTH + (SLIDER_RANGE - 1) * SMALLCHAR_WIDTH * s->range) - 2, y - 2, 12, 20, button);
}
...
=================
*/
static sfxHandle_t SpinControl_Key(menulist_s* s, int32 key)
{
    sfxHandle_t sound;
...
void Menu_AddItem(menuframework_s* menu, void* item)
{
    ...
    if (!(itemptr->flags & QMF_NODEFAULTINIT)) {
        ...
        default:
            Com_Error(ERR_DROP, va("Menu_Init: unknown type %d", itemptr->type));
        }
    }
...
{
    void (*callback)(void* self, int32 notification);
...
=================
*/
void Menu_SetCursor(menuframework_s* m, int32 cursor)
{
...
** slot.
*/
void Menu_AdjustCursor(menuframework_s* m, int32 dir)
{
...
=================
*/
sfxHandle_t Menu_DefaultKey(menuframework_s* m, int32 key)
{
    sfxHandle_t   sound = 0;
    menucommon_s* item;
    int32         cursor_prev;
...
void Menu_Cache()
{
    ...
    menu_in_sound = S_RegisterSound("sound/misc/menu1.wav", false);
    menu_move_sound = S_RegisterSound("sound/misc/menu2.wav", false);
    menu_out_sound = S_RegisterSound("sound/misc/menu3.wav", false);
    menu_buzz_sound = S_RegisterSound("sound/misc/menu4.wav", false);
    weaponChangeSound = S_RegisterSound("sound/weapons/change.wav", false);
