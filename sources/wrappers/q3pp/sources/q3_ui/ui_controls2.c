...
=================
*/
static void Controls_UpdateModel(int32 anim)
{
...
static void Controls_DrawKeyBinding(void* self)
{
    ...
    if (b1 == -1) {
        ...
        Q_strncpyz(name, Key_KeynumToString(b1), 32);
        Q_strupr(name);

        b2 = g_bindings[a->generic.id].bind2;
        if (b2 != -1) {
            Q_strncpyz(name2, Key_KeynumToString(b2), 32);
            Q_strupr(name2);
    ...
    if (c) {
        ...
        if (s_controls.waitingforkey) {
            ...
            UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.80, "Waiting for new key ... ESCAPE to cancel", UI_SMALLFONT | UI_CENTER | UI_PULSE, color_white);
        } else {
            UI_DrawChar(x, y, 13, UI_CENTER | UI_BLINK | UI_SMALLFONT, text_color_highlight);
            UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.78, "Press ENTER or CLICK to change", UI_SMALLFONT | UI_CENTER, color_white);
            UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.82, "Press BACKSPACE to clear", UI_SMALLFONT | UI_CENTER, color_white);
        }
    } else {
...
{
    UI_DrawString(SCREEN_WIDTH * 0.50, SCREEN_HEIGHT * 0.80, "Use Arrow Keys or CLICK to change", UI_SMALLFONT | UI_CENTER, color_white);
}
...
=================
*/
static void Controls_GetKeyAssignment(const char* command, int32* twokeys)
{
    int32 count;
    int32 j;
    char  b[256];
    ...
    for (j = 0; j < 256; j++) {
        if (const char* value = Key_GetBinding(j)) {
            Q_strncpyz(b, value, 256);
        } else {
            *b = 0;
        }
...
=================
*/
static sfxHandle_t Controls_MenuKey(int32 key)
{
    int32   id;
    int32   i;
    bool    found;
...
=================
*/
static void Controls_MenuEvent(void* ptr, int32 event)
{
...
=================
*/
static void Controls_ActionEvent(void* ptr, int32 event)
{
...
static void Controls_MenuInit()
{
    ...
    Cvar_VariableStringBuffer("name", (char*) s_controls.name.string, 16);
    Q_CleanStr((char*) s_controls.name.string);
