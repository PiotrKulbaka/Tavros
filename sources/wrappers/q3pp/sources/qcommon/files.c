...
static tavros::core::logger logger("files");
...
typedef struct
{
    ...
    int32          checksum;                // regular checksum
    int32          pure_checksum;           // checksum for pure
    int32          numfiles;                // number of files in pk3
    int32          referenced;              // referenced file flags
    int32          hashSize;                // hash table size (power of 2)
...
static int32         fs_packFiles; // total number of files in packs

static int32 fs_fakeChkSum;
static int32 fs_checksumFeed;
...
static int32 fs_numServerPaks;
static int32 fs_serverPaks[MAX_SEARCH_PATHS];     // checksums
static char* fs_serverPakNames[MAX_SEARCH_PATHS]; // pk3 names

// only used for autodownload, to make sure the client has at least
// all the pk3 files that are referenced at the server side
static int32 fs_numServerReferencedPaks;
static int32 fs_serverReferencedPaks[MAX_SEARCH_PATHS];     // checksums
...
================
*/
static int32 FS_HashFileName(const char* fname, int32 hashSize)
{
    int32 i;
    int32 hash;
    char  letter;
...
static bool FS_CreatePath(char* OSPath)
{
    ...
    // FIXME: is c: allowed???
    // TODO: if ( strstr( OSPath, ".." ) || strstr( OSPath, "::" ) ) {
    if (strstr(OSPath, "::")) {
        logger.warning("refusing to create relative path \"%s\"", OSPath);
        return true;
    }
...
===========
*/
char* FS_ShiftedStrStr(const char* string, const char* substring, int32 shift)
{
...
===========
*/
int32 FS_FOpenFileRead(const char* filename, fileHandle_t* file, bool uniqueFILE)
{
    searchpath_t* search;
    char*         netpath;
    pack_t*       pak;
...
=================
*/
int32 FS_Write(const void* buffer, int32 len, fileHandle_t h)
{
    int32  block, remaining;
    int32  written;
    uint8* buf;
    ...
    while (remaining) {
        ...
        if (written == 0) {
            if (!tries) {
                ...
                logger.info("FS_Write: 0 bytes written");
                return 0;
            }
        }

        if (written == -1) {
            logger.info("FS_Write: -1 bytes written");
            return 0;
        }
...
int32 FS_ReadFile(const char* qpath, void** buffer)
{
    ...
    isConfig = strstr(qpath, ".cfg") != nullptr;
    ...
    buf = (uint8*) Hunk_AllocateTempMemory(len + 1);
    *buffer = buf;
...
static pack_t* FS_LoadZipFile(char* zipfile, const char* basename)
{
    ...
    int32           i, len;
    int32           hash;
    int32           fs_numHeaderLongs;
    int32*          fs_headerLongs;
    char*           namePtr;
    ...
    fs_headerLongs = (int32*) Z_Malloc(gi.number_entry * sizeof(int32));
    ...
    pack->pure_checksum = Com_BlockChecksumKey(fs_headerLongs, 4 * fs_numHeaderLongs, (fs_checksumFeed));
...
static int32 FS_ReturnPath(const char* zname, char* zpath, int32* depth)
{
    int32 len, at, newdep;
...
==================
*/
static int32 FS_AddFileToList(const char* name, char* list[MAX_FOUND_FILES], int32 nfiles)
{
    int32 i;
...
===============
*/
char** FS_ListFilteredFiles(const char* path, const char* extension, const char* filter, int32* numfiles)
{
    int32         nfiles;
    char**        listCopy;
    ...
    int32         i;
    int32         pathLength;
    int32         extensionLength;
    int32         length, pathDepth, temp;
    pack_t*       pak;
    fileInPack_t* buildBuffer;
    ...
    for (search = fs_searchpaths; search; search = search->next) {
        ...
        if (search->pack) {
            ...
            if (fs_numServerPaks) {
                ...
                Sys_FreeFileList((char**) sysFiles);
            }
        }
    }
...
=================
*/
char** FS_ListFiles(const char* path, const char* extension, int32* numfiles)
{
...
=======================
 */
static uint32 Sys_CountFileList(const char** list)
{
    int32 i = 0;
...
static const char** Sys_ConcatenateFileLists(const char** list0, const char** list1, const char** list2)
{
    ...
    /* Create new list. */
    dst = cat = (const char**) Z_Malloc((totalLength + 1) * sizeof(char*));
...
{
    int32        nMods, i, j, nTotal, nLen, nPaks, nPotential, nDescLen;
    ...
    *listbuf = 0;
    ...
    for (i = 0; i < nPotential; i++) {
        ...
        if (Q_stricmp(name, "baseq3") && Q_stricmpn(name, ".", 1)) {
            ...
            Sys_FreeFileList((char**) pPaks); // we only use Sys_ListFiles to check wether .pk3 files are present
...
void FS_Dir_f()
{
    ...
    if (Cmd_Argc() < 2 || Cmd_Argc() > 3) {
        logger.info("usage: dir <directory> [extension]");
        return;
    }
...
===========
*/
int32 FS_PathCmp(const char* s1, const char* s2)
{
    int32 c1, c2;
...
================
*/
void FS_SortFileList(char** filelist, int32 numfiles)
{
    int32  i, j, k, numsortedfiles;
    char** sortedlist;
...
void FS_NewDir_f()
{
    ...
    if (Cmd_Argc() < 2) {
        logger.info("usage: fdir <filter>");
        logger.info("example: fdir *q3dm*.bsp");
        return;
    }
...
void FS_TouchFile_f()
{
    ...
    if (Cmd_Argc() != 2) {
        logger.info("Usage: touchFile <file>");
        return;
    }
...
static int32 QDECL paksort(const void* a, const void* b)
{
    char *aa, *bb;
...
static void FS_AddGameDirectory(const char* path, const char* dir)
{
    ...
    // done
    Sys_FreeFileList((char**) pakfiles);
}
...
static int32        fs_numServerReferencedPaks;
static int32        fs_serverReferencedPaks[MAX_SEARCH_PATHS];
...
void FS_PureServerSetLoadedPaks(const char* pakSums, const char* pakNames)
{
    ...
    if (fs_numServerPaks) {
        logger.debug("Connected to a pure server.");
    } else {
        if (fs_reordered) {
            // force a restart to make sure the search order will be correct
            logger.debug("FS search reorder is required");
