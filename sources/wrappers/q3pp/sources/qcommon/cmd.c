...
static tavros::core::logger logger("cmd");
...
int32 cmd_wait;
cmd_t cmd_text;
uint8 cmd_text_buf[MAX_CMD_BUFFER];
...
void Cbuf_AddText(const char* text)
{
    ...
    if (cmd_text.cursize + l >= cmd_text.maxsize) {
        logger.info("Cbuf_AddText: overflow");
        return;
    }
...
void Cbuf_InsertText(const char* text)
{
    ...
    if (len + cmd_text.cursize > cmd_text.maxsize) {
        logger.info("Cbuf_InsertText overflowed");
        return;
    }
...
void Cmd_Vstr_f()
{
    ...
    if (Cmd_Argc() != 2) {
        logger.info("vstr <variablename> : execute a variable command");
        return;
    }
...
For rcon use when you want to transmit without altering quotin
============
*/
char* Cmd_Cmd()
{
