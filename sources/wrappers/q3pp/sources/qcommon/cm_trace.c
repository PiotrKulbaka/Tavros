...
{
    int32      leafs[MAX_POSITION_LEAFS];
    int32      i;
    leafList_t ll;
...
==================
*/
void CM_TraceThroughTree(traceWork_t* tw, int32 num, float p1f, float p2f, vec3_t p1, vec3_t p2)
{
    cNode_t*  node;
    cplane_t* plane;
    float     t1, t2, offset;
...
==================
*/
void CM_Trace(trace_t* results, const vec3_t start, const vec3_t end, vec3_t mins, vec3_t maxs, clipHandle_t model, const vec3_t origin, int32 brushmask, int32 capsule, sphere_t* sphere)
{
    int32       i;
    traceWork_t tw;
    vec3_t      offset;
