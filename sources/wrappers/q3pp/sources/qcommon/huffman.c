...
int32 Huff_getBit(uint8* fin, int32* offset)
{
    int32 t;
    bloc = *offset;
...
static void add_bit(char bit, uint8* fout)
{
    if ((bloc & 7) == 0) {
        fout[(bloc >> 3)] = 0;
    }
...
static int32 get_bit(uint8* fin)
{
    int32 t;
    t = (fin[(bloc >> 3)] >> (bloc & 7)) & 0x1;
    bloc++;
...
static void send(node_t* node, node_t* child, uint8* fout)
{
    if (node->parent) {
...
void Huff_Decompress(msg_t* mbuf, int32 offset)
{
    int32  ch, cch, i, j, size;
    uint8  seq[65536];
    uint8* buffer;
    huff_t huff;
    ...
    for (j = 0; j < cch; j++) {
        ...
        Huff_addRef(&huff, (uint8) ch); /* Increment node */
    }
...
void Huff_Compress(msg_t* mbuf, int32 offset)
{
    int32  i, ch, size;
    uint8  seq[65536];
    uint8* buffer;
    ...
    for (i = 0; i < size; i++) {
        ...
        Huff_addRef(&huff, (uint8) ch); /* Do update */
    }
