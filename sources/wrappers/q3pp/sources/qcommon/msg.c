...
static tavros::core::logger logger("msg");
...
Handles uint8 ordering and avoids alignment errors
==============================================================================
*/
...
void MSG_WriteBits(msg_t* msg, int32 value, int32 bits)
{
    ...
    if (msg->oob) {
        if (bits == 8) {
            ...
            uint16* sp = (uint16*) &msg->data[msg->cursize];
            *sp = (value);
            msg->cursize += 2;
            msg->bit += 16;
        } else if (bits == 32) {
            uint32* ip = (uint32*) &msg->data[msg->cursize];
            *ip = (value);
            msg->cursize += 4;
            msg->bit += 8;
        } else {
...
int32 MSG_ReadBits(msg_t* msg, int32 bits)
{
    ...
    if (msg->oob) {
        if (bits == 8) {
            ...
            uint16* sp = (uint16*) &msg->data[msg->readcount];
            value = (*sp);
            msg->readcount += 2;
            msg->bit += 16;
        } else if (bits == 32) {
            uint32* ip = (uint32*) &msg->data[msg->readcount];
            value = (*ip);
            msg->readcount += 4;
            msg->bit += 32;
        } else {
...
void MSG_WriteData(msg_t* buf, const void* data, int32 length)
{
    int32 i;
    for (i = 0; i < length; i++) {
        MSG_WriteByte(buf, ((uint8*) data)[i]);
    }
}
...
void MSG_WriteString(msg_t* sb, const char* s)
{
    if (!s) {
        ...
        if (l >= MAX_STRING_CHARS) {
            logger.info("MSG_WriteString: MAX_STRING_CHARS");
            MSG_WriteData(sb, "", 1);
            return;
        }
...
void MSG_WriteBigString(msg_t* sb, const char* s)
{
    if (!s) {
        ...
        if (l >= BIG_INFO_STRING) {
            logger.info("MSG_WriteString: BIG_INFO_STRING");
            MSG_WriteData(sb, "", 1);
            return;
        }
        Q_strncpyz(string, s, sizeof(string));
...
int32 MSG_ReadByte(msg_t* msg)
{
    int32 c;

    c = (uint8) MSG_ReadBits(msg, 8);
...
int32 MSG_ReadShort(msg_t* msg)
{
    int32 c;

    c = (int16) MSG_ReadBits(msg, 16);
    if (msg->readcount > msg->cursize) {
        c = -1;
    }
...
void MSG_ReadData(msg_t* msg, void* data, int32 len)
{
    int32 i;

    for (i = 0; i < len; i++) {
        ((uint8*) data)[i] = MSG_ReadByte(msg);
    }
}
...
void MSG_WriteDeltaKey(msg_t* msg, int32 key, int32 oldV, int32 newV, int32 bits)
{
    if (oldV == newV) {
...
int32 MSG_ReadDeltaKey(msg_t* msg, int32 key, int32 oldV, int32 bits)
{
...
#define NETF(x) #x, (int32) & ((entityState_t*) 0)->x
...
// if (int32)f == f and (int32)f + ( 1<<(FLOAT_INT_BITS-1) ) < ( 1 << FLOAT_INT_BITS )
...
void MSG_WriteDeltaEntity(msg_t* msg, struct entityState_s* from, struct entityState_s* to, bool force)
{
    ...
    for (i = 0, field = entityStateFields; i < numFields; i++, field++) {
        fromF = (int32*) ((uint8*) from + field->offset);
        toF = (int32*) ((uint8*) to + field->offset);
        if (*fromF != *toF) {
            lc = i + 1;
        }
    }
...
void MSG_ReadDeltaEntity(msg_t* msg, entityState_t* from, entityState_t* to, int32 number)
{
    ...
    if (MSG_ReadBits(msg, 1) == 1) {
        ...
        if (cl_shownet->integer >= 2 || cl_shownet->integer == -1) {
            logger.info("%3i: #%-3i remove", msg->readcount, number);
        }
        return;
    }
    ...
    if (cl_shownet->integer >= 2 || cl_shownet->integer == -1) {
        print = 1;
        logger.info("%3i: #%-3i ", msg->readcount, to->number);
    } else {
        print = 0;
    }
...
// using the stringizing operator to save typing...
#define PSF(x) #x, (int32) & ((playerState_t*) 0)->x
...
void MSG_ReadDeltaPlayerstate(msg_t* msg, playerState_t* from, playerState_t* to)
{
    ...
    *to = *from;
    ...
    if (cl_shownet->integer >= 2 || cl_shownet->integer == -2) {
        print = 1;
        logger.info("%3i: playerstate ", msg->readcount);
    } else {
        print = 0;
    }
...
void MSG_initHuffman()
{
    ...
    for (i = 0; i < 256; i++) {
        for (j = 0; j < msg_hData[i]; j++) {
            Huff_addRef(&msgHuff.compressor, (uint8) i);   // Do update
            Huff_addRef(&msgHuff.decompressor, (uint8) i); // Do update
        }
    }
}
