...
static tavros::core::logger logger("cl_input");

uint32 frame_msec;
int32  old_com_frameTime;
...
void IN_KeyDown(kbutton_t* b)
{
    ...
    if (!b->down[0]) {
        ...
        logger.info("Three keys down for a button!");
        return;
    }
...
void CL_WritePacket()
{
    ...
    if (count > MAX_PACKET_USERCMDS) {
        ...
        logger.info("MAX_PACKET_USERCMDS");
    }
    if (count >= 1) {
    ...
    while (clc.netchan.unsentFragments) {
        logger.debug("#462 unsent fragments (not supposed to happen!)");
