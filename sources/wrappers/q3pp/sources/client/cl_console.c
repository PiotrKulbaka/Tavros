...
static tavros::core::logger logger("cl_console");

int32 g_console_field_width = 118;
...
{
    int32 i, j, width, oldwidth, oldtotallines, numlines, numchars;
    int16 tbuf[CON_TEXTSIZE];

    width = (CONSOLE_WIDTH / SMALLCHAR_WIDTH) - 2;
    ...
    if (width < 1) // video hasn't been initialized yet
    {
        ...
        Com_Memcpy(tbuf, con.text, CON_TEXTSIZE * sizeof(int16));
        for (i = 0; i < CON_TEXTSIZE; i++) {
            con.text[i] = (ColorIndex(COLOR_WHITE) << 8) | ' ';
        }
...
static void CL_ConsolePrint(const char* txt);
/*
================
Con_Init
================
...
void Con_DrawInput()
{
    ...
    Field_Draw(&g_consoleField, con.xadjust + 2 * SMALLCHAR_WIDTH, y, CONSOLE_WIDTH - 3 * SMALLCHAR_WIDTH, true);
}
...
void Con_DrawNotify()
{
    ...
    if (cls.keyCatchers & KEYCATCH_MESSAGE) {
        ...
        Field_BigDraw(&chatField, skip * BIGCHAR_WIDTH, v, CONSOLE_WIDTH - (skip + 1) * BIGCHAR_WIDTH, true);
