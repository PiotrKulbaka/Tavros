...
static tavros::core::logger logger("cl_parse");
...
==================
*/
void CL_DeltaEntity(msg_t* msg, clSnapshot_t* frame, int32 newnum, entityState_t* old, bool unchanged)
{
    entityState_t* state;
...
void CL_ParseSnapshot(msg_t* msg)
{
    ...
    if (newSnap.deltaNum <= 0) {
        ...
        if (!old->valid) {
            // should never happen
            logger.info("Delta from invalid frame (not supposed to happen!).");
        } else if (old->messageNum != newSnap.deltaNum) {
            // The frame that the server did the delta from
            // is too old, so we can't reconstruct it properly.
            logger.info("Delta frame too old.");
        } else if (cl.parseEntitiesNum - old->parseEntitiesNum > MAX_PARSE_ENTITIES - 128) {
            logger.info("Delta parseEntitiesNum too old.");
        } else {
...
void CL_ParseGamestate(msg_t* msg)
{
    ...
    while (1) {
        ...
        if (cmd == svc_configstring) {
            ...
            Com_Error(ERR_DROP, "CL_ParseGamestate: bad command uint8");
        }
    }
...
void CL_ParseDownload(msg_t* msg)
{
    ...
    if (!clc.download) {
        if (!*clc.downloadTempName) {
            logger.info("Server sending download, but no download was requested");
