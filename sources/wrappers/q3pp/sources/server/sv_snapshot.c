...
static tavros::core::logger logger("sv_snapshot");
...
typedef struct
{
    int32 numSnapshotEntities;
    int32 snapshotEntities[MAX_SNAPSHOT_ENTITIES];
...
=======================
*/
static int32 QDECL SV_QsortEntityNumbers(const void* a, const void* b)
{
    int32 *ea, *eb;
...
static void SV_AddEntitiesVisibleFromPoint(vec3_t origin, clientSnapshot_t* frame, snapshotEntityNumbers_t* eNums, bool portal)
{
    ...
    for (e = 0; e < sv.num_entities; e++) {
        ...
        if (ent->s.number != e) {
            logger.debug("FIXING ENT->S.NUMBER!!!");
            ent->s.number = e;
        }
...
static void SV_BuildClientSnapshot(client_t* client)
{
    ...
    for (i = 0; i < MAX_MAP_AREA_BYTES / 4; i++) {
        ((int32*) frame->areabits)[i] = ((int32*) frame->areabits)[i] ^ -1;
    }
...
static int32 SV_RateMsec(client_t* client, int32 messageSize)
{
    int32 rate;
    int32 rateMsec;
