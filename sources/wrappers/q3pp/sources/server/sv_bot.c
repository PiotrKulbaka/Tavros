...
static tavros::core::logger logger("sv_bot");
...
==================
*/
void QDECL BotImport_Print(int32 type, const char* fmt, ...)
{
    char    str[2048];
    va_list ap;
    ...
    case PRT_WARNING: {
        logger.warning(S_COLOR_YELLOW "Warning: %s", str);
        break;
    }
    case PRT_ERROR: {
        logger.error(S_COLOR_RED "Error: %s", str);
        break;
    }
    case PRT_FATAL: {
        logger.error(S_COLOR_RED "Fatal: %s", str);
        break;
    }
    case PRT_EXIT: {
...
==================
*/
void BotImport_Trace(bsp_trace_t* bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int32 passent, int32 contentmask)
{
    trace_t trace;
...
==================
*/
void BotImport_EntityTrace(bsp_trace_t* bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int32 entnum, int32 contentmask)
{
    trace_t trace;
...
==================
*/
int32 BotImport_PointContents(vec3_t point)
{
...
==================
*/
void BotImport_BSPModelMinsMaxsOrigin(int32 modelnum, vec3_t angles, vec3_t outmins, vec3_t outmaxs, vec3_t origin)
{
    clipHandle_t h;
    vec3_t       mins, maxs;
    float        max;
...
==================
*/
void* BotImport_GetMemory(int32 size)
{
    void* ptr;

    ptr = Z_TagMalloc(size, "botlib");
    return ptr;
}
...
=================
*/
void* BotImport_HunkAlloc(int32 size)
{
    if (Hunk_CheckMark()) {
...
==================
*/
void BotImport_DebugPolygonShow(int32 id, int32 color, int32 numPoints, vec3_t* points)
{
    bot_debugpoly_t* poly;
...
==================
*/
int32 BotImport_DebugLineCreate()
{
    vec3_t points[1];
...
==================
*/
void BotImport_DebugLineDelete(int32 line)
{
...
==================
*/
void BotImport_DebugLineShow(int32 line, vec3_t start, vec3_t end, int32 color)
{
...
int32 SV_BotLibSetup()
{
    ...
    if (!botlib_export) {
        logger.error("SV_BotLibSetup without SV_BotInitBotLib");
        return -1;
    }
...
==================
*/
int32 SV_BotGetSnapshotEntity(int32 client, int32 sequence)
{
    client_t*         cl;
