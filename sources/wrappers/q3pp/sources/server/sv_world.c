...
static tavros::core::logger logger("sv_world");
...
===============
*/
worldSector_t* SV_CreateworldSector(int32 depth, vec3_t mins, vec3_t maxs)
{
    worldSector_t* anode;
    vec3_t         size;
...
void SV_UnlinkEntity(sharedEntity_t* gEnt)
{
    ...
    logger.warning("SV_UnlinkEntity: not found in worldSector");
}
...
{
    worldSector_t* node;
    int32          leafs[MAX_TOTAL_ENT_LEAFS];
    int32          cluster;
    int32          num_leafs;
    int32          i, j, k;
    int32          area;
...
void SV_AreaEntities_r(worldSector_t* node, areaParms_t* ap)
{
    ...
    for (check = node->entities; check; check = next) {
        ...
        if (ap->count == ap->maxcount) {
            logger.info("SV_AreaEntities: MAXCOUNT");
            return;
        }
