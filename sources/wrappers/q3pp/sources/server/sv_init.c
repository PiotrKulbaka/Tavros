...
static tavros::core::logger logger("sv_init");
...
{
    int32     len, i;
    int32     maxChunkSize = MAX_STRING_CHARS - 24;
    client_t* client;
...
===============
*/
void SV_BoundMaxClients(int32 minimum)
{
...
void SV_SpawnServer(char* server, bool killBots)
{
    ...
    sv.checksumFeed = (((int32) rand() << 16) ^ rand()) ^ Com_Milliseconds();
    ...
    for (i = 0; i < sv_maxclients->integer; i++) {
        ...
        if (svs.clients[i].state >= CS_CONNECTED) {
            ...
            denied = (const char*) ClientConnect(i, false, isBot); // firstTime = false
            if (denied) {
    ...
    if (sv_pure->integer) {
        ...
        Cvar_Set("sv_paks", p);
        if (strlen(p) == 0) {
            logger.warning("sv_pure set but no PK3 files loaded");
        }
        p = FS_LoadedPakNames();
