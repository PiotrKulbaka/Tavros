...
typedef struct aas_trace_s
{
    ...
    int32  ent;        // entity blocking the trace
    int32  lastarea;   // last area the trace was in (zero if none)
    int32  area;       // area blocking the trace (zero if none)
    int32  planenum;   // number of the plane that was hit
} aas_trace_t;
...
{
    int32  valid;         // true if updated this frame
    int32  type;          // entity type
    int32  flags;         // entity flags
    float  ltime;         // local time
    float  update_time;   // time between last and current update
    int32  number;        // number of the entity
...
typedef struct aas_clientmove_s
{
    ...
    int32       endarea;      // area at end of movement prediction
    vec3_t      velocity;     // velocity at the end of movement prediction
    aas_trace_t trace;        // last trace
    int32       presencetype; // presence type at end of movement prediction
    int32       stopevent;    // event that made the prediction stop
    int32       endcontents;  // contents at the end of movement prediction
    float       time;         // time predicted ahead
    int32       frames;       // number of frames predicted ahead
} aas_clientmove_t;
...
typedef struct aas_altroutegoal_s
{
    vec3_t origin;
    int32  areanum;
    uint16 starttraveltime;
    uint16 goaltraveltime;
    uint16 extratraveltime;
} aas_altroutegoal_t;
...
typedef struct aas_predictroute_s
{
    ...
    int32  stopevent;      // event that made the prediction stop
    int32  endcontents;    // contents at the end of movement prediction
    int32  endtravelflags; // end travel flags
    int32  numareas;       // number of areas predicted ahead
    int32  time;           // time predicted ahead (in hundreth of a sec)
} aas_predictroute_t;
