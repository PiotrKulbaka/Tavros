...
static tavros::core::logger logger("g_mover");
...
void MatchTeam(gentity_t* teamLeader, moverState_t moverState, int32 time);
...
bool G_MoverPush(gentity_t* pusher, vec3_t move, vec3_t amove, gentity_t** obstacle)
{
    ...
    vec3_t     mins, maxs;
    pushed_t*  p;
    int32      entityList[MAX_GENTITIES];
    int32      listedEntities;
...
===============
*/
void SetMoverState(gentity_t* ent, moverState_t moverState, int32 time)
{
    vec3_t delta;
    float  f;
...
void Think_SetupTrainTargets(gentity_t* ent)
{
    ...
    if (!ent->nextTrain) {
        logger.info("func_train at %s with an unfound target", vtos(ent->r.absmin));
        return;
    }
    ...
    for (path = ent->nextTrain; path != start; path = next) {
        ...
        if (!path->target) {
            logger.info("Train corner at %s without a target", vtos(path->s.origin));
            return;
        }
        ...
        do {
            ...
            if (!next) {
                logger.info("Train corner at %s without a target path_corner", vtos(path->s.origin));
                return;
            }
...
{
    if (!self->targetname) {
        logger.info("path_corner with no targetname at %s", vtos(self->s.origin));
        G_FreeEntity(self);
        return;
    }
...
void SP_func_train(gentity_t* self)
{
    ...
    if (!self->target) {
        logger.info("func_train without a target at %s", vtos(self->r.absmin));
        G_FreeEntity(self);
        return;
    }
