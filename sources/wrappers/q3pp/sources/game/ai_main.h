...
typedef struct bot_goal_s
{
    ...
    int32  areanum;    // area number of the goal
    vec3_t mins, maxs; // mins and maxs of the goal
    int32  entitynum;  // number of the goal entity
    int32  number;     // goal number
    int32  flags;      // goal flags
    int32  iteminfo;   // item information
} bot_goal_t;
...
typedef struct bot_activategoal_s
{
    ...
    int32                      shoot;                    // true if bot has to shoot to activate
    int32                      weapon;                   // weapon to be used for activation
    vec3_t                     target;                   // target to shoot at to activate something
    vec3_t                     origin;                   // origin of the blocking entity to activate
    int32                      areas[MAX_ACTIVATEAREAS]; // routing areas disabled by blocking entity
    int32                      numareas;                 // number of disabled routing areas
    int32                      areasdisabled;            // true if the areas are disabled for the routing
...
{
    int32         inuse;                 // true if this state is used by a bot client
    int32         botthink_residual;     // residual for the bot thinks
    int32         client;                // client number of the bot
    int32         entitynum;             // entity number of the bot
    playerState_t cur_ps;                // current player state
    int32         last_eFlags;           // last ps flags
    usercmd_t     lastucmd;              // usercmd from last frame
    int32         entityeventTime[1024]; // last entity event time
    //
    bot_settings_t settings;                          // several bot settings
    int32          (*ainode)(struct bot_state_s* bs); // current AI node
    float          thinktime;                         // time the bot thinks this frame
    vec3_t         origin;                            // origin of the bot
    vec3_t         velocity;                          // velocity of the bot
    int32          presencetype;                      // presence type of the bot
    vec3_t         eye;                               // eye coordinates of the bot
    int32          areanum;                           // the number of the area the bot is in
    int32          inventory[MAX_ITEMS];              // string with items amounts the bot has
    int32          tfl;                               // the travel flags the bot uses
    int32          flags;                             // several flags
    int32          respawn_wait;                      // wait until respawned
    int32          lasthealth;                        // health value previous frame
    int32          lastkilledplayer;                  // last killed player
    int32          lastkilledby;                      // player that last killed this bot
    int32          botdeathtype;                      // the death type of the bot
    int32          enemydeathtype;                    // the death type of the enemy
    int32          botsuicide;                        // true when the bot suicides
    int32          enemysuicide;                      // true when the enemy of the bot suicides
    int32          setupcount;                        // true when the bot has just been setup
    int32          map_restart;                       // true when the map is being restarted
    int32          entergamechat;                     // true when the bot used an enter game chat
    int32          num_deaths;                        // number of time this bot died
    int32          num_kills;                         // number of kills of this bot
    int32          revenge_enemy;                     // the revenge enemy
    int32          revenge_kills;                     // number of kills the enemy made
    int32          lastframe_health;                  // health value the last frame
    int32          lasthitcount;                      // number of hits last frame
    int32          chatto;                            // chat to all or team
    ...
    int32          predictobstacles_goalareanum;      // last goal areanum the bot predicted obstacles for
    vec3_t         aimtarget;
    vec3_t         enemyvelocity;                     // enemy velocity 0.5 secs ago during battle
    vec3_t         enemyorigin;                       // enemy origin 0.5 secs ago during battle
    //
    int32 kamikazebody; // kamikaze body
    int32 proxmines[MAX_PROXMINES];
    int32 numproxmines;
    //
    int32 character; // the bot character
    int32 ms;        // move state of the bot
    int32 gs;        // goal state of the bot
    int32 cs;        // chat state of the bot
    int32 ws;        // weapon state of the bot
    //
    int32  enemy;            // enemy entity number
    int32  lastenemyareanum; // last reachability area the enemy was in
    vec3_t lastenemyorigin;  // last origin of the enemy in the reachability area
    int32  weaponnum;        // current weapon number
    vec3_t viewangles;       // current view angles
    vec3_t ideal_viewangles; // ideal view angles
    vec3_t viewanglespeed;
    //
    int32 ltgtype; // long term goal type
    // team goals
    int32      teammate;                 // team mate involved in this team goal
    int32      decisionmaker;            // player who decided to go for this goal
    int32      ordered;                  // true if ordered to do something
    float      order_time;               // time ordered to do something
    int32      owndecision_time;         // time the bot made it's own decision
    ...
    int32      teamtaskpreference;       // team task preference
    // last ordered team goal
    int32      lastgoal_decisionmaker;
    int32      lastgoal_ltgtype;
    int32      lastgoal_teammate;
    bot_goal_t lastgoal_teamgoal;
    // for leading team mates
    int32      lead_teammate;    // team mate the bot is leading
    ...
    int32      numteammates;                                 // number of team mates
    int32      redflagstatus;                                // 0 = at base, 1 = not at base
    int32      blueflagstatus;                               // 0 = at base, 1 = not at base
    int32      neutralflagstatus;                            // 0 = at base, 1 = our team has flag, 2 = enemy team has flag, 3 = enemy team dropped the flag
    int32      flagstatuschanged;                            // flag status changed
    int32      forceorders;                                  // true if forced to give orders
    int32      flagcarrier;                                  // team mate carrying the enemy flag
    int32      ctfstrategy;                                  // ctf strategy
    ...
    int32           patrolflags;                             // patrol flags
} bot_state_t;
...
void BotEntityInfo(int32 entnum, aas_entityinfo_t* info);

extern float floattime;
#define FloatTime() floattime

// from the game source
void QDECL       BotAI_Print(int32 type, const char* fmt, ...);
void QDECL QDECL BotAI_BotInitialChat(bot_state_t* bs, const char* type, ...);
void             BotAI_Trace(bsp_trace_t* bsptrace, vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int32 passent, int32 contentmask);
int32            BotAI_GetClientState(int32 clientNum, playerState_t* state);
int32            BotAI_GetEntityState(int32 entityNum, entityState_t* state);
int32            BotAI_GetSnapshotEntity(int32 clientNum, int32 sequence, entityState_t* state);
int32            BotTeamLeader(bot_state_t* bs);
