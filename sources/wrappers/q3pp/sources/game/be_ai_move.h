...
typedef struct bot_initmove_s
{
    ...
    int32  or_moveflags; // values ored to the movement flags
} bot_initmove_t;

// NOTE: the ideal_viewangles are only valid if MFL_MOVEMENTVIEW is set
typedef struct bot_moveresult_s
{
    int32  failure;          // true if movement failed all together
    int32  type;             // failure or blocked type
    int32  blocked;          // true if blocked by an entity
    int32  blockentity;      // entity blocking the bot
    int32  traveltype;       // last executed travel type
    int32  flags;            // result flags
    int32  weapon;           // weapon used for movement
