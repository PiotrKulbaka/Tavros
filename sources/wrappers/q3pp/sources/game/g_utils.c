...
static tavros::core::logger logger("g_utils");
...
================
*/
static int32 G_FindConfigstringIndex(const char* name, int32 start, int32 max, bool create)
{
    int32 i;
...
void G_UseTargets(gentity_t* ent, gentity_t* activator)
{
    ...
    while ((t = G_Find(t, FOFS(targetname), ent->target)) != NULL) {
        if (t == ent) {
            logger.warning("Entity used itself.");
        } else {
            if (t->use) {
        ...
        if (!ent->inuse) {
            logger.info("entity was removed while using targets");
            return;
        }
    }
}
...
char* vtos(const vec3_t v)
{
    ...
    Com_sprintf(s, 32, "(%i %i %i)", (int32) v[0], (int32) v[1], (int32) v[2]);
