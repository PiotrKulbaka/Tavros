...
static tavros::core::logger logger("g_client");
...
============
*/
static void ClientCleanName(const char* in, char* out, int32 outSize)
{
    int32 len, colorlessLen;
    char  ch;
    char* p;
    int32 spaces;

    // save room for trailing null uint8
    outSize--;
...
{
    gentity_t* ent;
    int32      teamTask, teamLeader, team, health;
    char*      s;
    ...
    if (ent->r.svFlags & SVF_BOT) {
        s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%i\\skill\\%s\\tt\\%d\\tl\\%d", client->pers.netname, team, model, headModel, c1, c2, client->pers.maxHealth, client->sess.wins, client->sess.losses, Info_ValueForKey(userinfo, "skill"), teamTask, teamLeader);
    } else {
...
void ClientSpawn(gentity_t* ent)
{
    ...
    int32              persistant[MAX_PERSISTANT];
    gentity_t*         spawnPoint;
    int32              flags;
    int32              savedPing;
    //    char    *savedAreaBits;
    int32 accuracy_hits, accuracy_shots;
    int32 eventSequence;
    char  userinfo[MAX_INFO_STRING];
