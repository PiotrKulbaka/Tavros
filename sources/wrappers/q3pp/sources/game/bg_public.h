...
typedef struct
{
    ...
    void  (*trace)(trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int32 passEntityNum, int32 contentMask);
    int32 (*pointcontents)(const vec3_t point, int32 passEntityNum);
} pmove_t;
...
typedef enum
{
    ...
    PERS_EXCELLENT_COUNT,     // two successive kills in a int16 amount of time
...
typedef struct animation_s
{
    int32 firstFrame;
    int32 numFrames;
    int32 loopFrames;  // 0 to numFrames
    int32 frameLerp;   // msec between frames
    int32 initialLerp; // msec to get to first frame
    int32 reversed;    // true if animation is reversed
    int32 flipflop;    // true if animation should flipflop back to base
} animation_t;
...
typedef struct gitem_s
{
    ...
    int32      quantity;     // for ammo how much, or duration of powerup
