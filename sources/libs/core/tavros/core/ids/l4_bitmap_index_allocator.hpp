...
namespace tavros::core
{
     ...
     *
     * Manages up to 16,777,216 indices (64x64x64x64) using a four-level bitmap
     ...
     *
     * @par Complexity
     * - `allocate()`   - O(1), four `first_zero_bit` calls down the hierarchy
     * - `deallocate()` - O(1), direct bit clear with up to three parent updates
     * - `contains()`   - O(1), single bit test in l4
     * - `find_first()` - O(1), four `first_set_bit` calls down the hierarchy
     * - `next_after()` - O(1) amortized, uses l3/l2/l1 to jump empty blocks
     *
     * @par Memory layout
     * All four bitmaps are cache-line aligned (64 bytes).
     * Total size: 8 + 512 + 32768 + 2097152 + padding = 2,097 KB (~2.1 MB).
     *
     * @note All indices are 0-based.
     * @note Capacity is fixed at compile time by @ref k_capacity.
     * @note Due to its size this allocator should be heap-allocated.
     *
     * @see index_allocator_base
     * @see index_allocator_concept
     */
    class l4_bitmap_index_allocator final : public index_allocator_base<l4_bitmap_index_allocator>
    {
        friend class index_allocator_base<l4_bitmap_index_allocator>;
    ...
    public:
        constexpr static index_t k_capacity = static_cast<index_t>(k_l4_map_size * k_bits_per_word);
...
#pragma warning(pop)
    ...
    static_assert(index_allocator_concept<l4_bitmap_index_allocator>);
