...
namespace tavros::resources
{
    ...
    image_codec::~image_codec() = default;
    ...
    image_codec::pixels_view image_codec::decode(tavros::core::buffer_view<uint8> packed_pixels, uint32 required_channels, bool y_flip)
    {
        ...
        return {w, h, required_channels, stride, {m_buffer.data(), required_size}};
    }

    tavros::core::buffer_view<uint8> image_codec::encode(const image_codec::pixels_view& pixels, image_format fmt, bool y_flip)
    {
        struct ctx_t
        {
        ...
        auto  w = static_cast<int32>(pixels.width);
        auto  h = static_cast<int32>(pixels.height);
        auto  c = static_cast<int32>(pixels.channels);
        auto* p = reinterpret_cast<const unsigned char*>(pixels.pixels.data());
        auto  s = static_cast<int32>(pixels.stride);

        int32 success = 0;

        switch (fmt) {
        case image_format::png:
            success = stbi_write_png_to_func(write_fn, &ctx, w, h, c, p, s);
            break;

        case image_format::jpg:
            success = stbi_write_jpg_to_func(write_fn, &ctx, w, h, c, p, s);
            break;
        ...
        case image_format::tga:
            ...
            success = stbi_write_tga_to_func(write_fn, &ctx, w, h, c, p);
            break;
        ...
        case image_format::bmp:
            ...
            success = stbi_write_bmp_to_func(write_fn, &ctx, w, h, c, p);
            break;
        default:
            TAV_UNREACHABLE();
        }
