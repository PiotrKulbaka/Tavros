...
#include <tavros/text/rich_text.hpp>
...
namespace tavros::text
{
    ...
    public:
         ...
         */
        // static geometry::aabb2 layout(core::buffer_span<glyph_data> glyphs, const layout_params& params) noexcept
        //{
        //     using glyph_t = glyph_data;
        ...
        //            // While is not space
        //            while (it < end && !it->is_space) {
        //                auto  scale = it->glyph_size;
        //                auto& font_info = it->font->get_font_metrics();

        //                cur_word.width += it->advance_x + it->kern_x;
        ...
        //                cur_word.right_spaces_width += it->advance_x + it->kern_x;
        //                cur_word.ascent_y = max_abs(cur_word.ascent_y, font_info.ascent_y * scale);
        //                cur_word.descent_y = max_abs(cur_word.descent_y, font_info.descent_y * scale);
        //                cur_word.line_gap_y = max_abs(cur_word.line_gap_y, font_info.line_gap_y * scale);
        ...
        //            auto& info = it->font->get_glyph_info(it->idx);
        //            auto  scale = it->glyph_size;

        //            auto layout_pos = baseline_pos + it->bearing - (math::vec2(0.0f, info.metrics.size.height)) * scale;
        //            it->layout.min = layout_pos;
        //            it->layout.max = layout_pos + info.metrics.size * scale;
        //            it->bounds.min = math::vec2(baseline_pos.x, baseline_pos.y - line.ascent_y);
        //            it->bounds.max = math::vec2(baseline_pos.x + it->advance_x, baseline_pos.y - line.descent_y);

        //            if (it->is_space && remaining_gaps > 0) {
        //                if (is_leading_spaces) {
        //                    baseline_pos.x += it->advance_x + it->kern_x;
        //                } else {
        //                    it->layout.min.x += additional_space / 2.0f;
        //                    it->layout.max.x += additional_space / 2.0f;
        //                    it->bounds.max.x += additional_space;

        //                    baseline_pos.x += it->advance_x + it->kern_x + additional_space;
        //                    --remaining_gaps;
        //                }
        //            } else {
        //                baseline_pos.x += it->advance_x + it->kern_x;
        ...
        /*template<tavros::core::ArchetypeWith<glyph_metrics_c> T>
        static geometry::aabb2 layout(T& data, const layout_params& params) noexcept
        {
            geometry::aabb2 container;
