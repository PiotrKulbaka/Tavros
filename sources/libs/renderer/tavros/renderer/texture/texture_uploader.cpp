...
namespace tavros::renderer
{
    ...
    void texture_uploader::upload_2d_level(rhi::texture_handle gpu_tex, assets::image_view im, uint32 mip_level, gpu_stage_buffer& stage, rhi::command_queue& cmd)
    {
    ...
    void texture_uploader::upload_2d(rhi::texture_handle gpu_tex, assets::image_view base, core::buffer_view<assets::image> levels, gpu_stage_buffer& stage, rhi::command_queue& cmd)
    {
        upload_2d_level(gpu_tex, base, 0, stage, cmd);
        auto sz = static_cast<uint32>(levels.size());
        for (uint32 i = 0; i < sz; ++i) {
            upload_2d_level(gpu_tex, levels[i], i + 1, stage, cmd);
        }
    }
